...
# define a function to help with for loops
def load_data(list, csv_file, column_name):
    with open(csv_file, newline='') as csv_info:
        reader = csv.DictReader(csv_info)
        for row in reader:
            list.append(row[column_name])
        return list

age = load_data(insured_age, 'D:\csv_documents\insurance.csv', 'age')
sex = load_data(insured_sex, 'D:\csv_documents\insurance.csv', 'sex')
bmi = load_data(insured_bmi, 'D:\csv_documents\insurance.csv', 'bmi')
children = load_data(insured_children, 'D:\csv_documents\insurance.csv', 'children')
smoker = load_data(insured_smoker, 'D:\csv_documents\insurance.csv', 'smoker')
region = load_data(insured_region, 'D:\csv_documents\insurance.csv', 'region')
charges = load_data(insured_charges, 'D:\csv_documents\insurance.csv', 'charges')
